...
export default {
  ...
  methods: {
    ...
    setupChart: function () {
      const sortedData = this.rawData.sort();
      const categoryPositions = sortedData.map((d, i) => d.min);
      const xRangeToIndex = (val) => {
        const bin = this.rawData.find((x) => x.min <= val && x.max >= val);
        return bin ? bin.min : val; // gunakan nilai min sebagai posisi
      };
      ...
      this.defaultOptions = {
        ...
        yaxis: {
          min: 0,
          max: Math.max(...this.rawData.map((d) => d.value)) + 2,
          labels: {
            formatter: (val) => `${val}`,
          },
          title: {
        ...
        annotations: {
          yaxis: [
            {
              y: xRangeToIndex(this.batasBawah),
              borderColor: "#00E396",
              strokeDashArray: 4,
              label: {
                borderColor: "#00E396",
            ...
            {
              y: xRangeToIndex(this.batasAtas),
              borderColor: "#FF4560",
              strokeDashArray: 4,
              label: {
                borderColor: "#FF4560",
        ...
        // title: {
        //   text: this.titleChart || "Line Chart Distribusi",
        //   align: "center",
        // },
        colors: this.colors,
      };
