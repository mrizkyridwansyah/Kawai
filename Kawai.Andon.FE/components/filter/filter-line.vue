...
export default {
  ...
  emits: ["update:modelValue", "update:lineName"],
  props: [
    "modelValue",
    "type",
    "label",
  ...
  computed: {
    ...
    displayLabel() {
     if (this.isOpen) return "LineName";
  ...
  methods: {
    change: function (v) {
      if (this.onSelect) this.onSelect(v);
       const selected = this.list.find(x => x.LineCode === v) || null;

      this.$emit("update:modelValue", v);
      this.$emit("update:lineName", selected?.LineName || "");

      // console.log("Selected LineCode:", v);
      // console.log("Selected LineName:", selected?.LineName || "");
    },
    search: function (q) {
      this.load(q, null);
    },
    ...
    load: function (q = "", d = "") {
      ...
      this.debounce = setTimeout(() => {
        this.$http
          .get(
            `/andon/filter/ddlline?keyword=${q || ""}&ids=${
              d || ""
            }`
          )
          .then((p) => {
