  ...
  <div class="row">
    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12">
      <input-multiselect
        ...
        :searchable="true"
        :label="displayLabel"
        track-by="SupplierCode"
        trackBy="SupplierCode"
        :hide-selected="true"
...
export default {
  ...
  emits: ["update:modelValue", "update:supplier-name"],
  props: [
    "modelValue",
    "type",
    "label",
  ...
  computed: {
    ...
    selectedItem: function () {
      return this.list.find((x) => x.SupplierCode === this.tempValue) || null;
    },
    displayLabel() {
      if (this.isOpen) return "DDLDescription";
      return this.tempValue ? "SupplierCode" : "DDLDescription";
    },
  },
  watch: {
  ...
  methods: {
    change: function (v) {
      ...
      const selected = this.list.find((x) => x.SupplierCode === v) || null;

      this.$emit("update:modelValue", v);
      this.$emit("update:supplier-name", selected?.SupplierName || "");
    ...
    load: function (q = "", d = "") {
      ...
      this.debounce = setTimeout(() => {
        this.$http
          .get(
            `/andon/filter/ddlsupplier?keyword=${q || ""}&ids=${
              d || ""
            }`,
          )
          .then((p) => {
            if (d && p.data.Data.length > 0) {
              this.tempValue = p.data.Data[0]?.SupplierCode;
             this.$emit("update:supplier-name", p.data.Data[0].SupplierName);
            }
             if (
              ...
              this.$emit("update:modelValue", "ALL");
               this.$emit("update:supplier-name", "ALL");
            }
