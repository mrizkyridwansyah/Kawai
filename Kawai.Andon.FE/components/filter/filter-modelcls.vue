...
export default {
  ...
  emits: ["update:modelValue", "update:modelDescs"],
  props: [
    "modelValue",
    "type",
    "label",
  ...
  methods: {
    change: function (v) {
      if (this.onSelect) this.onSelect(v);
       const selected = this.list.find(x => x.ModelCls === v) || null;

      this.$emit("update:modelValue", v);
      this.$emit("update:modelDescs", selected?.ModelDescs || "");

      // console.log("Selected Model_Cls:", v);
    ...
    load: function (q = "", d = "") {
      ...
      this.debounce = setTimeout(() => {
        this.$http
          .get(
            `/andon/filter/ddlmodel?keyword=${q || ""}&ids=${
              d || ""
            }`
          )
          .then((p) => {
            if (d && p.data.Data.length > 0) {
              this.tempValue = p.data.Data[0]?.ModelCls;
            }
