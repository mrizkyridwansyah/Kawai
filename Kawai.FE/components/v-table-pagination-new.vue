      ...
      <div class="d-flex">
        <div>
          <select
            ...
            @change="changeLength"
          >
            <option v-for="i in [5, 10, 25, 50, 100]" :key="i" :value="i">
              {{ i }}
            </option>
          </select>
        </div>
...
export default {
  ...
  computed: {
    totalPages: function () {
      return Math.ceil(this.model.Filtered / this.model.Length) || 1;
    },
    pages: function () {
  ...
  watch: {
    table: {
      ...
      handler: function (after) {
        if (!after) return;
        this.model.Page = after.Page || 1;
        this.model.Length = after.Length || 10;
        this.model.Filtered = after.Filtered || 0;
        this.model.Total = after.Total || 0;
      },
    },
  },
  mounted: function () {
    if (this.table) {
      this.model.Page = this.table.Page || 1;
      this.model.Length = this.table.Length || 10;
      this.model.Filtered = this.table.Filtered || 0;
      this.model.Total = this.table.Total || 0;
    }
  },
  methods: {
    goNext: function () {
      if (this.model.Page < this.totalPages) {
        ...
        this.$emit("page-change", this.model.Page);
      }
    },
    goPrev: function () {
    ...
    goPage: function (v) {
      if (this.disabledGoPage(v)) return;

      this.model.Page = parseInt(v);
      if (this["pageChange"]) this["pageChange"](this.model.Page);
      this.$emit("page-change", this.model.Page);
    },
    ...
    changeLength: function (e) {
      ...
      if (this["lengthChange"]) this["lengthChange"](val);
      this.$emit("length-change", val);
    },
  },
};
</script>
