    ...
    <tr>
      <td :style="this.styleCode">
        <input-multiselect
          ...
          :searchable="true"
          :label="displayLabel"
          track-by="ParentItemCode"
          trackBy="ParentItemCode"
          :hide-selected="true"
...
export default {
  ...
  computed: {
    ...
    displayLabel() {
      ...
      return this.tempValue ? "ParentItemCode" : "DDLDescription";
    },
  },
  ...
  methods: {
    change: function (v) {
      ...
      const selected = this.list.find((x) => x.ParentItemCode === v);
      console.log(selected);
      this.itemDescription = selected?.ParentItemName || "";
    ...
    load: function (q = "", d = "") {
      ...
      this.debounce = setTimeout(() => {
        this.$http
          .get(`/production-unschedule/ddl-parentitem-search?keyword=${q || ""}&ids=${d || ""}`) //&itemGroup=01&brand=${this.brand || ''}&itemType=${this.itemType || ''}&itemCls=${this.itemCls || ''}`)
          .then((p) => {
            if (d && p.data.Data.length > 0) {
              this.tempValue = p.data.Data[0]?.ParentItemCode;
              this.itemDescription = p.data.Data[0]?.ParentItemName || "";
