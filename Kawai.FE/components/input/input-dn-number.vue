  ...
  methods: {
    ...
    load: function (q = "", d = "") {
      ...
      this.debounce = setTimeout(() => {
        this.$http
          .get(
            `/receipt/dn-ddlsearch?keyword=${q || ""}&ids=${d || ""}&factory=${
              this.factoryCode
            }&supplier=${this.supplierCode || ""}&status=${
              this.status || ""
            }${this.periodFrom ? "&periodFrom=" +this.$func.asUtcStringDateOnly(new Date(this.periodFrom)): ""}${this.periodUntil ? "&periodUntil=" + this.$func.asUtcStringDateOnly(new Date(this.periodUntil)) : ""}`,
          )
          .then((p) => {
