...
export default {
  ...
  computed: {
    ...
    selectedItem: function () {
      return this.list.find((x) => x.IPAddress === this.tempValue) || null;
    },
    displayLabel() {
      if (this.isOpen) return "DDLDescription";
      return this.tempValue ? "IPAddress" : "DDLDescription";
    },
  },
  watch: {
  ...
  methods: {
    ...
    load: function (q = "", d = "") {
      ...
      this.debounce = setTimeout(() => {
        this.$http
          .get(
            `/area/ddlsearchprinter?keyword=${q || ""}&ids=${
              d || ""
            }`,
          )
          .then((p) => {
            if (d && p.data.Data.length > 0) {
              this.tempValue = p.data.Data[0]?.IPAddress;
            }
