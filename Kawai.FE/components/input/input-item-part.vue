<template>
  <div>
    ...
    :clear-on-select="false"
      ...
      :placeholder="placeholder || `Search Item Part`"
      :searchable="true" 
      label="ItemName" 
      track-by="ItemCode" 
      trackBy="ItemCode" 
...
export default {
  ...
  props: [
    ...
    , 'disabled', 'multiple', 'class', 'itemType', 'brand', 'itemCls'
  ],
  data: () => ({
    isLoading: false,
    list: [],
  ...
  methods: {
    ...
    load: function (q = '', d = '') {
      ...
      this.debounce = setTimeout(() => {
        this.$http.get(`/item/ddlsearch?keyword=${q || ''}&ids=${d || ''}&itemGroup=ALL&brand=${this.brand || ''}&itemType=${this.itemType || ''}&itemCls=${this.itemCls || ''}`)
          .then(p => {
