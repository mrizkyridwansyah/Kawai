...
[Route("api/trolley")]
[ApiController]
public class TrolleyController : HahaController
{
    private readonly ITrolleyRepository _trolleyRepository;
    private readonly DataLogger _logger;

    public TrolleyController(ITrolleyRepository trolleyRepository, DataLogger logger)
    {
    ...
    {
        var results = await _trolleyRepository.GetAll(parameter);
    ...
    {
        var results = await _trolleyRepository.GetDDL(keyword);
        if (!string.IsNullOrEmpty(ids))
        {
            var idList = ids.Split(',').Select(id => id.Trim()).ToList();
            results = results.Where(x => idList.Contains(x.TrolleyCode)).ToList();
        }
    ...
    {
        var result = await _trolleyRepository.GetData(id);
        return Success(result);
    }

    [HttpPost("create")]
    public async Task<IActionResult> Create([FromBody] Trolley model)
    {
        await _trolleyRepository.Create(model, Auth.User.UserID);

        var after = await _trolleyRepository.Capture(model.TrolleyCode);
        await _logger.SaveDataLog(new DataLogDto
        {
            DocumentType = "Master Trolley",
    ...
    [HttpPatch("update")]
    public async Task<IActionResult> Update([FromBody] Trolley model)
    {
        var before = await _trolleyRepository.Capture(model.TrolleyCode);
        await _trolleyRepository.Update(model, Auth.User.UserID);
        var after = await _trolleyRepository.Capture(model.TrolleyCode);
    ...
    {
        var before = await _trolleyRepository.Capture(id);
        await _trolleyRepository.Remove(id, Auth.User.UserID);
        await _logger.SaveDataLog(new DataLogDto
        {
    ...
    public async Task<IActionResult> ExportExcel([FromBody] RequestParameter parameter)
    {
        ...
        List<string> headers = ["Trolley Code", "Description", "Type",  "Active", "Last Update", "Last User"];
        ...
        foreach (var result in results)
        {
            ...
            ExcelHelper.SetCell(row, colIdx, result.TrolleyCode);
            colIdx++;
            ExcelHelper.SetCell(row, colIdx, result.Description);
            colIdx++;
            ExcelHelper.SetCell(row, colIdx, result.Trolley_ClsDescs);
            colIdx++;
            ExcelHelper.SetCell(row, colIdx, result.IsActive);
            colIdx++;
