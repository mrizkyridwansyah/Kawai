...
[Authorize]
[Route("api/trolleycls")]
[ApiController]
public class TrolleyClsController : HahaController
{
    private readonly ITrolleyClsRepository _trolleyclsRepository;
    private readonly DataLogger _logger;

    public TrolleyClsController(ITrolleyClsRepository trolleyclsRepository, DataLogger logger)
    {
        _trolleyclsRepository = trolleyclsRepository;
        _logger = logger;
    }
    ...
    {
        var results = await _trolleyclsRepository.GetAll(parameter);
    ...
    {
        var result = await _trolleyclsRepository.GetData(id);
        return Success(result);
    }

    [HttpPost("create")]
    public async Task<IActionResult> Create([FromBody] TrolleyCls model)
    {
        await _trolleyclsRepository.Create(model, Auth.User.UserID);

        var after = await _trolleyclsRepository.Capture(model.Trolley_Cls);
        await _logger.SaveDataLog(new DataLogDto
        {
            DocumentType = "Trolley Cls",
            EntityId = model.Trolley_Cls,
    ...
    [HttpPatch("update")]
    public async Task<IActionResult> Update([FromBody] TrolleyCls model)
    {
        var before = await _trolleyclsRepository.Capture(model.Trolley_Cls);
        await _trolleyclsRepository.Update(model, Auth.User.UserID);
        var after = await _trolleyclsRepository.Capture(model.Trolley_Cls);
    ...
    {
        var before = await _trolleyclsRepository.Capture(id);
        await _trolleyclsRepository.Remove(id, Auth.User.UserID);
        await _logger.SaveDataLog(new DataLogDto
        {
            DocumentType = "Trolley Cls",
            EntityId = id,
    ...
    public async Task<IActionResult> ExportExcel([FromBody] RequestParameter parameter)
    {
        ...
        List<string> headers = ["Trolley Cls", "Description", "Qty",   "Last Update", "Last User"];
        ...
        foreach (var result in results)
        {
            ...
            ExcelHelper.SetCell(row, colIdx, result.Trolley_Cls);
            colIdx++;
