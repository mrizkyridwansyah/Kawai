CREATE VIEW [dbo].[vw_StockHeader]
AS
SELECT			Stock_Year,Stock_Month,
				AreaCode = RTRIM(ISNULL(TBL.AreaCode,'')) ,
				Area = RTRIM(ISNULL(MA.AreaName,'')) ,
				WarehouseCode = RTRIM(ISNULL(TBL.Warehouse_Code,'')) ,
        ...
        FROM    ( 
				...
				RTRIM(ISNULL(TBL.AreaCode,'')),
				RTRIM(ISNULL(MA.AreaName,'')),
				RTRIM(ISNULL(TBL.Warehouse_Code,'')),
