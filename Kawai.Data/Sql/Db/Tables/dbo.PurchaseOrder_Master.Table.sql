...
CREATE TABLE [PurchaseOrder_Master](
	...
	[PriceContract_Cls] [char](1) NULL,
 CONSTRAINT [PK_PurchaseOrder_Master] PRIMARY KEY CLUSTERED 
(
	[PO_No] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, FILLFACTOR = 80, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
ALTER TABLE [PurchaseOrder_Master]  WITH NOCHECK ADD  CONSTRAINT [FK_PurchaseOrder_Master_Trade_Master] FOREIGN KEY([Supplier_Code])
REFERENCES [Trade_Master] ([Trade_Code])
ON UPDATE CASCADE
ON DELETE CASCADE
GO
ALTER TABLE [PurchaseOrder_Master] CHECK CONSTRAINT [FK_PurchaseOrder_Master_Trade_Master]
GO
