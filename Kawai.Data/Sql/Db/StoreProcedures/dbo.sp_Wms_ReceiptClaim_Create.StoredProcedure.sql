...
CREATE procedure [dbo].[sp_Wms_ReceiptClaim_Create]
...
begin
	...
	select * from MaterialNGClaimHeader

	declare @overReceiptPO varchar(max) = 
	(
		select STRING_AGG(a.ClaimNo, ', ') From MaterialNGClaimHeader a
		inner join MaterialNGClaimDetail cc on a.ClaimID = cc.ClaimID
		inner join Item_Master mi on cc.ItemCode = mi.Item_Code
		left join 
		(
			select 
		...
		) rcpSum on a.ClaimNo = rcpSum.PONumber and cc.ItemCode = rcpSum.ItemCode
		inner join @Details dtl on a.ClaimNo = dtl.PONumber and cc.ItemCode = dtl.ItemCode
		where a.TotalQty - (isnull(rcpSum.ReceiptQty, 0) + dtl.ReceiptQty) < 0
	)
	...
	begin try
		...
		--left join MaterialNGClaimHeader pod on dtl.ItemCode = pod.Item_Code and dtl.PONumber = pod.PO_No
		--where hd.Id = @newid
