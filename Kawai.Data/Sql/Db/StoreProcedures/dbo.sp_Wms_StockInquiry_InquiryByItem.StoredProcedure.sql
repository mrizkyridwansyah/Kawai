...
GO
CREATE OR ALTER PROCEDURE [sp_Wms_StockInquiry_InquiryByItem]
--declare
	-- PARAMETER WAJIB
	@Page int = 1,
	@Length int = 10,
		...
		(
			...
			(
				...
				(
					...
					inner join 
					(
						select sdx.WarehouseCode, mw.WarehouseName 
						...
						) mw on sdx.WarehouseCode = mw.WarehouseCode
						where 1=1
						and sdx.Qty > 0
						...
						group by sdx.WarehouseCode, mw.WarehouseName
						'+ @sqlSort +'
					...
					left join Item_Master mi on sdt.ItemCode = mi.Item_Code
					left join 
					(
