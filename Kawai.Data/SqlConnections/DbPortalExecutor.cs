...
public class DbPortalExecutor
{
    private readonly IConnectionFactory _connectionFactory;
    public DbPortalExecutor(IConnectionFactory connectionFactory)
    {
    ...
    public async Task<IEnumerable<T>> QueryListAsync<T>(string sql, object? param = null, CommandType commandType = CommandType.StoredProcedure)
    {
        try
        {
            using var conn = _connectionFactory.GetDbPortalConnection();
            conn.Open();
    ...
    {
        using var connection = _connectionFactory.GetDbPortalConnection();
        connection.Open();
